...
const submissions = [
  ...
  { id: 'MV-2026-0098', applicant: 'João Augusto Lima', proposal: 'Memórias da Vila Esperança', notice: 'Memória Viva', date: '04 set, 16h08', documents: '2 de 3', status: 'Documentação pendente' },
...
function App() {
  const [activeView, setActiveView] = useState<'agenda' | 'editais' | 'criar-edital' | 'administracao' | 'revisar-inscricao'>('agenda');
    ...
    return (
      <main className="app-shell">
        ...
        <section className="review-page" aria-labelledby="review-title">
          ...
          <aside className="review-summary"><span className={`submission-status ${status === 'Validada' ? 'validated' : status === 'Documentação pendente' ? 'pending' : ''}`}>{status}</span><strong>{selectedSubmission.notice}</strong><span>Proponente: {selectedSubmission.applicant}</span></aside>
          ...
          <aside className="decision-panel"><p className="eyebrow">Decisão administrativa</p><h2>Validar inscrição</h2><p>Confirme que os dados e documentos atendem aos requisitos do edital.</p><label>Observação para o histórico<textarea rows={4} placeholder="Adicione uma observação, se necessário." /></label><div><button className="review-button" type="button" onClick={() => { setValidationStatus({ ...validationStatus, [selectedSubmission.id]: 'Documentação pendente' }); setActiveView('administracao'); }}>Solicitar ajuste</button><button className="validate-button" type="button" onClick={() => { setValidationStatus({ ...validationStatus, [selectedSubmission.id]: 'Validada' }); setActiveView('administracao'); }}>Validar inscrição</button></div></aside>
        </section>
      </main>
    );
  }
    ...
    return (
      <main className="app-shell">
        <header className="site-header">
          <button className="brand brand-button" type="button" onClick={() => setActiveView('agenda')} aria-label="Cultura em movimento, início"><span className="brand-mark">C</span><span>Cultura<br />em movimento</span></button>
          <nav aria-label="Navegação principal"><button type="button" onClick={() => setActiveView('agenda')}>Agenda</button><button type="button" onClick={() => setActiveView('editais')}>Editais</button><button className="nav-active" type="button">Administração</button></nav>
        ...
        <section className="admin-page" aria-labelledby="admin-title">
          ...
          <div className="admin-stats" aria-label="Resumo das submissões"><div><strong>28</strong><span>submissões recebidas</span></div><div><strong>11</strong><span>aguardando validação</span></div><div><strong>4</strong><span>com pendência documental</span></div><div><strong>13</strong><span>validadas</span></div></div>
          <div className="submission-toolbar"><div className="submission-filters"><button className="filter active" type="button">Todas</button><button className="filter" type="button">Em análise</button><button className="filter" type="button">Pendências</button><button className="filter" type="button">Validadas</button></div><label className="search-field">Buscar<input type="search" placeholder="Nome ou protocolo" /></label></div>
          <div className="submission-list">
            ...
            {submissions.map((submission) => {
              ...
              return <article className="submission-row" key={submission.id}>
                ...
                <span className={`submission-status ${status === 'Validada' ? 'validated' : status === 'Documentação pendente' ? 'pending' : ''}`}>{status}</span>
                <div className="submission-actions"><button type="button" className="review-button" onClick={() => { setSelectedSubmission(submission); setActiveView('revisar-inscricao'); }}>Ver inscrição</button>{status !== 'Validada' && <button type="button" className="validate-button" onClick={() => setValidationStatus({ ...validationStatus, [submission.id]: 'Validada' })}>Validar</button>}</div>
              </article>;
            })}
          </div>
        </section>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="form-grid">
                  ...
                  <label>Tipo de inscrição *
                    <select name="applicant-type" value={applicantType} onChange={(event) => setApplicantType(event.target.value === 'cnpj' ? 'cnpj' : 'cpf')}>
                      <option value="cpf">Pessoa física (CPF)</option>
                      <option value="cnpj">Pessoa jurídica (CNPJ)</option>
                    </select>
                  </label>
                  <label>{applicantType === 'cpf' ? 'CPF' : 'CNPJ'} *
                    <input name={applicantType} required inputMode="numeric" maxLength={applicantType === 'cpf' ? 11 : 14} pattern="[0-9]+" title="Digite somente números" placeholder={applicantType === 'cpf' ? '00000000000' : '00000000000000'} />
                  </label>
